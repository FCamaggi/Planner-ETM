...
import {
  CustomEventModal,
  CustomEventData,
} from './components/CustomEventModal';
import {
  timeToMinutes,
  checkOverlap,
  generateEtmLink,
...
export default function App() {
  ...
  const [editingEvent, setEditingEvent] = useState<CustomEventData | null>(
    null
  );
  ...
  const handleSaveCustomEvent = (data: CustomEventData) => {
    if (data.id) {
      ...
      setCustomEvents((prev) =>
        prev.map((e) =>
          e.id === data.id ? { ...e, ...data, isCustom: true } : e
        )
      );
    } else {
      // Add new
  ...
  const agendaItems = useMemo(() => {
    ...
    const itemsWithConflicts = sorted.map((item) => {
      const conflicts = sorted.filter(
        (other) => other.id !== item.id && checkOverlap(item, other)
      );
      return {
        ...item,
        ...
        <div className="hidden print:block p-6">
          {agendaItems.length === 0 ? (
            <p className="text-center text-gray-500">
              No hay eventos en tu agenda para este día.
            </p>
          ) : (
              ...
              <tbody>
                {agendaItems
                  ...
                  .map((item) => {
                    ...
                    const link = item.isCustom
                      ? '-'
                      : generateEtmLink(item.stage, item.title, item.id);
          ...
          {agendaItems.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center text-gray-400 opacity-60 print:hidden">
              ...
              <p className="text-lg font-medium">
                Tu agenda está vacía para este día
              </p>
              <p className="text-sm">
                Selecciona eventos del panel lateral para comenzar
              </p>
            </div>
          ) : (
                ...
                {agendaItems.map((item) => {
                  ...
                  if (item.isGap) {
                    return (
                      <div
                        key={item.id}
                        className="relative pl-10 py-2 group/gap print:hidden"
                      >
                              ...
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                {item.suggestions
                                  ...
                                  .map((sugg: any) => (
                                    <div
                                      key={sugg.id}
                                      className="bg-white border border-gray-200 rounded-lg p-2.5 shadow-sm hover:border-etm-300 hover:shadow-md transition-all flex justify-between items-center gap-2"
                                    >
                                      <div className="min-w-0 flex-1">
                                        ...
                                        <h4
                                          className="text-xs font-bold text-gray-800 leading-tight truncate"
                                          title={sugg.title}
                                        >
                                          {sugg.title}
                                        </h4>
                  ...
                  return (
                    ...
                    >
                      {/* Timeline Node */}
                      <div
                        className={`absolute left-[11px] top-5 w-2.5 h-2.5 rounded-full border-2 bg-white z-10 transition-colors print:border-gray-400 ${
                          item.hasConflict
                            ? 'border-amber-500 ring-4 ring-amber-100'
                            : item.isCustom
                            ? 'border-rose-500 group-hover:border-rose-600'
                            : 'border-etm-600 group-hover:border-etm-700'
                        }`}
                      ></div>

                      {/* Card */}
                      <div
                        className={`rounded-xl border transition-all duration-200 mb-4 print:border-gray-300 print:shadow-none ${
                          item.hasConflict
                                ...
                                <div className="text-sm text-gray-500 mb-3 flex flex-wrap gap-1">
                                  {item.speakers.map(
                                    (s: string, idx: number) => (
                                      <span
                                        key={idx}
                                        className="after:content-[','] last:after:content-[''] pr-1"
                                      >
                                        {s}
                                      </span>
                                    )
